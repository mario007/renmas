...
def build_scene2():
    ...
    c_props = {"type":"pinhole", "eye":(0.278, 0.275, -0.789), "lookat":(0,0,1), "distance":800}
...
def build_scene():
    s_props = {"type":"random", "width": WIDTH, "height": HEIGHT, "nsamples": NSAMPLES}
    s =ren.create_sampler(s_props)
    ...
    l_props = {"type":"point", "name": "light1", "position":(8,15,8), "spectrum":(0.99,0.99,0.99)}
    l = ren.create_light(l_props)


    m_props = {"type":"lambertian", "R":(0.67, 0.44, 0.0)}
    m = ren.create_material("m1")
    ren.add_brdf("m1", m_props)

    m_props = {"type":"lambertian", "R":(0.1, 0.1, 0.6)}
    ren.add_brdf("m1", m_props)
